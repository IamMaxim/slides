      ...
      {step >= 2 &&
        colsToRender.map((c) => (
          <text
            ...
            letterSpacing="0.1em"
          >
            {c === 0 ? 'ВХОД' : c === cols - 1 ? 'ВЫХОД' : `СКРЫТЫЙ ${c}`}
          </text>
        ))}
    </svg>
  );
...
export const networkSlide: Slide = {
  ...
  render: ({ step }) => (
    <Split
      ...
      left={
        <Stack gap={20}>
          <Eyebrow>строительный блок · 02</Eyebrow>
          <SlideTitle>Сложи их в стопку — получишь сеть.</SlideTitle>
          <Stack gap={14} style={{ marginTop: 12 }}>
            <Build step={step} appearAt={1}>
              <BodyText>
                Поставь много нейронов в ряд — и получишь <span style={{ color: 'var(--accent)' }}>слой</span>.
                Каждый нейрон в слое видит одни и те же входы, но у каждого свои веса.
              </BodyText>
            </Build>
            <Build step={step} appearAt={2}>
              <BodyText>
                Подай выходы одного слоя на вход следующему, повтори — и получишь глубокую сеть.
                Нейроны в середине выучивают промежуточные признаки, которые никто явно не задавал.
              </BodyText>
            </Build>
            <Build step={step} appearAt={3}>
              <BodyText size="lg">
                <span style={{ color: 'var(--ink)' }}>Это аппроксиматор функций.</span>{' '}
                При достаточном числе нейронов и правильных весах он может выучить почти любое отображение входа в выход.
              </BodyText>
            </Build>
          </Stack>
        </Stack>
